...
/**
 *cette classe permet de mettre en oeuvre l' algorithme simple retour arrière 
 * cas des variables reelles
 * @author NDJAMA
 */
public class SimRetAr {
    ...
    */
    ...
    /**
     * le rôle de cette méthode est de simuler notre algorithme simple retour arriere
     ...
     * @return un boolean
     */
    public  boolean simretar(ArrayList<Variable> variable,Contrainte con,Variable varchoisie)
    {
        String carac;
           ...
           //On itère sur le domaine de la variable choisie
            for (indexval[varchoisie.getIndex()] = 0;indexval[varchoisie.getIndex()] < varchoisie.getDomaine().size(); indexval[varchoisie.getIndex()]++) {
               ...
               //Au cas où nous avons une solution
                if(simretar(variable, con,varchoisie))
                {   
    ...
    public void launch()
    {
        ...
        String nom = "outputs/resultat SRA.jcsp";

	    try {
			fich = new PrintWriter(new BufferedWriter(new FileWriter(nom)));
                        fich.println("\t\t\t\t *****************NOUVELLE EVALUATION (ALGORITHME SRA) DU "+dateStr+"*****************");
        ...
        //on affiche et construit notre arbre jusqu'a ce qu'on atteint le nombre d'iterations totales calculées ci-dessus
          result.clear();
          simretar(BienvenueController.variable, BienvenueController.con,new Variable());
